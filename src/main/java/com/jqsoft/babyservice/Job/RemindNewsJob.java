...
//    @Scheduled(cron = "${job.cron.remindNewsJob}")
    public void remindNewsJob() {
        ...
        // 获取所有需要提醒的体检计划
        Date now = new Date();
        List<ExaminationInfo> examinationInfos = examinationInfoMapper.getNeedRemindExaminationInfoList();
        if (CollectionUtils.isNotEmpty(examinationInfos)) {
            ...
            for (ExaminationInfo info : examinationInfos) {
                ...
                if (DateUtils.isSameDay(now, info.getExaminationDate())) {
                    ...
                    // 逾期 发送钉钉工作通知，并记录数据库
                    String addDays = DateUtil.differentDaysByDate(now,info.getExaminationDate()).toString();
                    addDays = "0".equals(addDays) ? "" : "+" + addDays ;
                    news.setTitle(StringUtils.join(info.getBabyName()," - ", age, addDays,"儿童健康体检签到"));
                    news.setContext(String.format(remindTemplate2, age, age));
                    news.setType((byte)2);
